"""Train config_011 (best LSTM config) with GloVe embeddings, class weighting, and early stopping (NO LR scheduling)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train best LSTM config (config_011) with GloVe embeddings, class weighting, and early stopping"
    )
    parser.add_argument(
        "--fold",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("experiments/lstm_config011_glove"),
    ...
    # Best configuration from tuning (config_011) with GloVe embeddings
    # NO learning rate scheduling (since it reduced PR-AUC)
    config = {
        "name": "config011_glove",
        "description": f"Best config (011) with GloVe + class weights + early stopping (NO LR scheduling): hidden_dim=256, max_length=200, num_layers=2, lr=0.0005, dropout=0.4",
        "params": {
            "hidden_dim": 256,
            "max_length": 200,
            "num_layers": 2,
            ...
            "embedding_path": str(args.embeddings_path),  # Pre-trained GloVe embeddings
            "freeze_embeddings": args.freeze_embeddings,  # Allow fine-tuning by default
            "use_class_weights": True,  # Class weighting for imbalanced labels
            "early_stopping_patience": args.patience,
            "early_stopping_metric": "pr_auc",
            # NO lr_scheduler_type - keeping fixed learning rate
        },
    }

    print("=" * 70)
    print("TRAINING BEST LSTM CONFIG (config_011) WITH GLOVE EMBEDDINGS")
    print("=" * 70)
    ...
    print(f"Learning rate: 0.0005 (fixed, NO scheduling)")
    ...
    try:
        ...
        improved_pr_auc = 0.579760  # config_011 with class weights + early stopping (NO LR scheduling)
        lr_scheduler_pr_auc = 0.0  # Will be updated if we have this value
        
        print(f"\n📈 Comparison:")
        print(f"   Baseline (15 epochs):           {baseline_pr_auc:.6f}")
        print(f"   With class weights (14 epochs): {improved_pr_auc:.6f}")
        print(f"   With GloVe + class weights:      {pr_auc:.6f}")
        ...
        if improvement_vs_improved > 0:
            print(f"   ✅ vs Improved (no LR): +{improvement_vs_improved:.6f} ({improvement_vs_improved/improved_pr_auc*100:.2f}%)")
        else:
            print(f"   ⚠️  vs Improved (no LR): {improvement_vs_improved:.6f} ({improvement_vs_improved/improved_pr_auc*100:.2f}%)")
        ...
        if pr_auc > 0.6:
            ...
            print(f"\n💡 GloVe embeddings improved results!")
